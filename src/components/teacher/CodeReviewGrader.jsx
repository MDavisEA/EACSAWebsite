        ...
        {groups.map((g, i) => {
          ...
          return (
            ...
            >
              ...
              {/* A prior grade was archived when they turned this in again -
                  see reopenMine. Distinct from "N attempts" above, which
                  counts raw rows (old data, or a resubmission that beat a
                  since-fixed bug) rather than a real graded-then-redone
                  cycle. */}
              {s.resubmitted && (
        ...
        <DialogContent className="max-w-[95vw] w-[95vw] max-h-[93vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 flex-wrap">
              ...
              {current?.resubmitted && (
                <Badge variant="outline" className="text-blue-700 border-blue-300 font-normal">
                  Resubmitted
                </Badge>
              )}
            </DialogTitle>
          ...
          {current?.resubmitted && <PreviousVersionsPanel submissionId={current.id} />}
...
// since most submissions never have any.
function PreviousVersionsPanel({ submissionId }) {
        ...
        return (
          <div key={v.id}>
            ...
            >
              ...
              <Arrow className={`w-3.5 h-3.5 ml-auto transition-transform ${open ? "rotate-90" : ""}`} />
            </button>
            {open && (
